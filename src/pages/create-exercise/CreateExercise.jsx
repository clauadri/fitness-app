...
const CreateExercise = () => {
    ...
    const createExercise = (data, navigateTo)=>async(dispatch)=>{   
        try {
            console.log(data);
            const result =await API.post("/exercise/create", data)
            console.log(result);
  ...
  return (
    <div>
        ...
        <label>
            ...
            <input type="number" name="rest"{...register("rest",{required:true})}/>
        </label>
        <label>
            <p>Imagen</p>
            <input type="file" name="img"{...register("img",{required:true})}/>
        </label>
